...
export function TwilioProvisioningWidget({ practiceName, onProvisioned, onSkip }: Props) {
  ...
  useEffect(() => {
    if (process.env.NEXT_PUBLIC_TWILIO_TEST_MODE === 'true') {
      toast.warning('Prototype mode — sub-account creation and number purchase are simulated. Add production Twilio credentials to provision real numbers.', {
        duration: 8000,
        id: 'twilio-test-mode',
      });
    }
  ...
  const createSubAccount = async () => {
    ...
    try {
      ...
      if (json._testMode) {
        toast.info('Sub-account simulated (test mode) — no real Twilio account created.');
      } else {
      ...
      setSubError(msg);
      toast.error(`Sub-account failed: ${msg}`);
    } finally {
      setSubLoading(false);
    }
  };
  ...
  const searchNumbers = async () => {
    ...
    try {
      ...
      if (json.numbers.length === 0) {
        ...
        toast.warning(`No numbers found in area code ${areaCode}. Try a different one.`);
      } else {
      ...
      setSearchError(msg);
      toast.error(`Number search failed: ${msg}`);
    } finally {
  ...
  const purchaseNumber = async () => {
    ...
    try {
      ...
      setPurchased(result);
      onProvisioned({ subAccount, purchasedNumber: result });
      if (json._testMode) {
        toast.info('Number purchase simulated (test mode) — no real number charged.');
      } else {
        toast.success(`${result.phoneNumber} provisioned successfully!`);
      }
    } catch (err) {
      ...
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-6 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Form fields auto-filled below</p>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 flex-1 min-w-0">
            ...
            {!subAccount ? (
              ...
              <div className="flex items-center gap-2 text-xs text-emerald-400">
                ...
                Sub-account created · Status: {subAccount.status}
              </div>
            )}
          </div>
        </div>
                ...
                {searchResults.map((num) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {num.capabilities.voice && <Mic size={12} className="text-slate-500" aria-label="Voice" />}
                      {num.capabilities.sms && <MessageSquare size={12} className="text-slate-500" aria-label="SMS" />}
                      <div className={cn(
        ...
        <div className={cn('flex gap-4', (!selectedNumber || purchased) && 'opacity-40 pointer-events-none')}>
          ...
          <div className="flex flex-col gap-3 flex-1 min-w-0">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Finalises purchase in your Twilio account</p>
            </div>
