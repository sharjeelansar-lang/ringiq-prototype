...
function DataRow({ label, value, mono = false }: { label: string; value?: string | number | null; mono?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2.5 border-b border-slate-800/30 last:border-0 group">
      <span className="text-[10px] font-semibold tracking-widest uppercase text-slate-600 w-36 shrink-0">
        {label}
      </span>
...
function Panel({ title, icon: Icon, accent = 'cyan', children }: {
  ...
  return (
    <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 overflow-hidden">
      <div className="flex items-center gap-2.5 px-5 py-3 border-b border-slate-800/50 bg-slate-900/60">
...
function Skeleton() {
  return (
    <div className="flex flex-col gap-5 animate-pulse">
      <div className="h-20 bg-slate-900/60 rounded-xl border border-slate-800/50" />
      <div className="grid grid-cols-2 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="rounded-xl border border-slate-800/60 bg-slate-900/30 p-4 flex flex-col gap-2.5">
            <div className="h-3.5 bg-slate-800 rounded w-28" />
            {[...Array(3)].map((__, j) => (
              <div key={j} className="h-2.5 bg-slate-800/60 rounded w-full" style={{ opacity: 1 - j * 0.25 }} />
            ))}
          </div>
        ))}
      </div>
    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex items-center gap-4 px-8 py-5 border-b border-slate-800/70 bg-slate-950/80 backdrop-blur-sm shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-200
              transition-colors px-2 py-1.5 rounded-lg hover:bg-slate-800 shrink-0"
          >
            <ArrowLeft size={15} />
            Back
          </button>
          ...
          {office ? (
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <div className="min-w-0">
                <h1 className="text-lg font-bold text-white tracking-tight truncate">{office.name}</h1>
                <p className="text-[11px] font-mono text-slate-600 mt-0.5">{office._id}</p>
              </div>
          ...
          {office && (
            <button
              ...
              className="ml-auto text-slate-600 hover:text-slate-300 transition-colors p-2 rounded-lg hover:bg-slate-800 shrink-0"
              title="Refresh"
            >
              <RefreshCw size={14} />
            </button>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {error && (
            <div className="flex items-center gap-3 px-5 py-4 rounded-xl border border-red-500/20 bg-red-500/5 text-red-400">
          ...
          {!loading && office && (
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                {/* Timezone */}
                <div className="flex items-center gap-3 px-5 py-4 rounded-xl border border-slate-800/60 bg-slate-900/50">
                  <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-violet-500/8 border border-violet-500/20 shrink-0">
                    <Globe size={14} className="text-violet-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold tracking-widest uppercase text-slate-600 mb-1">Timezone</p>
                    <p className="text-sm font-mono text-slate-300 truncate">{office.tzName || '—'}</p>
                  </div>
                </div>

                {/* CPMID */}
                <div className="flex items-center gap-3 px-5 py-4 rounded-xl border border-slate-800/60 bg-slate-900/50">
                  <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-amber-500/8 border border-amber-500/20 shrink-0">
                    <Hash size={14} className="text-amber-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold tracking-widest uppercase text-slate-600 mb-1">CPM ID</p>
                    <p className="text-sm font-mono text-slate-300 truncate">{office.ehr?.cpmid || '—'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Panel title="Working Hours" icon={Clock} accent="rose">
                  {office.workingHours && Object.keys(office.workingHours).length > 0
                    ...
                    : <div className="py-3 text-xs text-slate-600">No hours configured</div>
                  }
                ...
                <Panel title="Provisioned Lines" icon={PhoneCall} accent="cyan">
                  {office.twilioNumbers?.length ? (
                    ...
                    <div className="py-3 text-xs text-slate-600">No Twilio numbers provisioned</div>
                  )}
                </Panel>
