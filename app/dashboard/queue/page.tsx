...
import {
  ...
  Calendar, ArrowRight, X, Check, Clock, Inbox,
} from 'lucide-react';
...
const STATUS_FILTER_TABS: { key: 'all' | QueueStatus; label: string }[] = [
  { key: 'all',      label: 'All' },
...
export default function QueuePage() {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      ...
      setItems(json.queue as QueueItem[]);
    } catch (err) {
      setFetchError(err instanceof Error ? err.message : 'Failed to fetch queue');
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
          {fetchError && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 10, border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.04)', fontSize: 13, color: T.red, marginBottom: 16 }}>
              <span style={{ fontWeight: 600 }}>Failed to load queue:</span>
                      ...
                      <div style={{ padding: '0 20px 20px', borderTop: `1px solid ${T.border}`, background: T.bg, animation: 'fade-up .2s ease-out both' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, padding: '20px 0 20px' }}>
                          ...
                          <div style={{ background: T.surface, borderRadius: 10, border: `1px solid ${T.border}`, padding: '16px 18px' }}>
                            ...
                            {item.notes && (
                              <p style={{ fontSize: 12, color: T.muted, lineHeight: 1.5, margin: 0, fontStyle: 'italic' }}>
                                "{item.notes}"
                              </p>
                            )}
                          </div>
                        ...
                        {item.status === 'pending' && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 4 }}>
                            ...
                            <button
                              ...
                              disabled={isActing}
                              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 9, background: T.tealFd, color: T.teal, border: `1.5px solid ${T.tealBd}`, fontSize: 13, fontWeight: 600, cursor: isActing ? 'not-allowed' : 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}
                            >
                              <Check size={13} />
                              Approve Only
                            </button>
                            ...
                            <span style={{ fontSize: 12, color: T.light, marginLeft: 8 }}>
                              Approved automatically when wizard is completed
                            </span>
                          </div>
                        )}
                        ...
                        {item.status === 'approved' && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 4 }}>
                            <button
                              onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/businesses/new?prospectId=${item.id}`); }}
                              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 9, background: T.brand, color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 14px rgba(39,73,147,0.25)', fontFamily: 'inherit' }}
                            >
                              <ArrowRight size={13} /> Open Wizard
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleReject(item.id); }}
                              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px', borderRadius: 9, background: 'none', color: T.red, border: `1.5px solid ${T.redBd}`, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
                            >
                              <X size={12} /> Undo Approval
                            </button>
                          </div>
                        )}
